...
const UserBanner = ({ owner_id, owner_name, user_exp, avatar_id }) => {
    ...
    return (
        <Link to={`/home/${owner_id}`} className="flex items-center static max-w-screen-md h-28 mx-auto my-2 border-t-neutral-500 bg-slate-100 hover:bg-slate-200 shadow hover:transform hover:scale-[1.01] transition duration-100">
            <img src={avatar_id} alt="Avatar" className="static w-20 h-auto rounded-full ml-4" />
            <div className='relative container ml-6 flex items-center'>
                <h2 className="static flex items-center text-xl font-medium text-black truncate w-1/2">{owner_name}</h2>
